// globalConfig.js
		...
		console.log("last:" + _lastHighscore);
		...
		function renderMenus() {
			...
			switch (state.menus.active) {
				...
				case MENU_SCORE:
					...
					let xhr_recode = new XMLHttpRequest();
					xhr_recode.onreadystatechange = function()
					{
						if (xhr_recode.readyState == 4)
						{

							if (xhr_recode.status == 200)
							{
								// 请求发送成功，根据服务器端返回的响应结果来进行相应的操作
								//console.log(xhr_recode.responseText);
							}
					...
					xhr_recode.open('POST', '../recode.php', true);
					xhr_recode.setRequestHeader("Content-type","application/x-www-form-urlencoded");
					//console.log(state.game.score);
					let data = "gametype=cubes&username=" + username + "&score=" + state.game.score;
					xhr_recode.send(data);
