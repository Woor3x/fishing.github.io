...
ig.module('game.menus.base').requires('impact.font').defines(function () {
    ...
    Menu = ig.Class.extend({
        clearColor: null,
        name: null,
        font: new ig.Font('fonts/avenir-36-white.png'),
        fontSelected: new ig.Font('fonts/avenir-36-orange.png'),
        current: 0,
        itemClasses: [],
        items: [],
        scale: 1,
...
ig.module('game.menus.about').requires('game.menus.base').defines(function () {
    ...
    MenuAbout = Menu.extend({
        background: new ig.Image('background/stars.jpg'),
        credits: new ig.Image('title/credits.png'),
        scale: 0.75,
        scroll: 0,
...
ig.module('game.menus.interstitial').requires('game.menus.base').defines(function () {
    ...
    MenuInterstitial = Menu.extend({
        ...
        scale: 0.75,
        banner: new ig.Image('nfn-phoboslab-works.png'),
        init: function () {
            this.parent();
        ...
        update: function () {
            this.parent();
            var bx = ((ig.system.width - this.banner.width) / 2), by = 64, bw = this.banner.width, bh = this.banner.height;
        ...
        draw: function () {
            this.parent();
            var x = (ig.system.width - this.banner.width) / 2, y = 64;
    ...
    StatsView = ig.Class.extend({
        ...
        load: function () {
            if (window.Cocoon || window.Ejecta) {
                ...
                ig.xhr('backend/', {load: true}, this.receivedData.bind(this));
            }
        },
        ...
        drawGraph: function (x, y) {
            ...
            for (var i = 0; i < numLabels; i++) {
                ...
                var xp = x + this.width, yp = y + this.height - this.height * v;
            ...
            for (var i = 0; i < max; i++) {
                ...
                var rx = x + (rw - 3) * v, ry = y + this.height - (this.height - 3) * (d.score / this.scoreScale);
...
ig.module('game.menus.game-over').requires('game.menus.base', 'game.menus.interstitial', 'game.menus.stats').defines(function () {
    ...
    MenuGameOver = Menu.extend({
        ...
        scale: 0.75,
        personalBestBadge: new ig.Image('ui/personal-best-badge.png'),
        fontTitle: new ig.Font('fonts/avenir-36-blue.png'),
        separatorBar: new ig.Image('ui/bar-blue.png'),
        init: function () {
...
ig.module('game.menus.pause').requires('game.menus.base').defines(function () {
    ...
    MenuPause = Menu.extend({
        scale: 0.8, clearColor: 'rgba(0,0,0,0.8)', init: function () {
            this.parent();
        ...
        }, itemClasses: [MenuItemSoundVolume, MenuItemMusicVolume, MenuItemResume, MenuItemBack], draw: function () {
            this.width = ig.system.width / this.scale;
            var ctx = ig.system.context;
            this.parent();
            var s = 0.85;
...
ig.module('game.menus.detailed-stats').requires('game.menus.base', 'game.xhr').defines(function () {
    DetailedStats = ig.Class.extend({
        container: null, closeButton: null, chart: null, maxScore: 0, games: [], scoreScale: 1, init: function () {
            ...
            setTimeout(function () {
                ...
                setTimeout(function () {
                    ...
                    ig.xhr('backend/', {loadAll: true}, that._initChart.bind(that));
                }, 400);
            }, 16);
            ...
            var nw = this.content.clientWidth, nh = this.content.clientHeight - 64;
            ...
            var width = canvas.width, height = canvas.height - 128;
            var ctx = canvas.getContext('2d');
            ctx.fillStyle = '#fff';
            ...
            for (var i = 0; i < numLabels; i++) {
                var v = 1 / (numLabels - 1) * i;
                var label = Math.round(v * this.scoreScale);
                var xp = x + rw + 32, yp = y + height - height * v;
                ctx.fillStyle = '#555';
            ...
            for (var i = 0; i < this.data.length; i++) {
                var g = this.data[i];
                var v = 1 / (this.data.length - 1) * i || 0;
                var rx = x + (rw - 3) * v, ry = y + height - (height - 3) * (g.score / this.scoreScale);
                ctx.fillRect(rx, ry, 3, 3);
                if (lastp) {
                    ctx.beginPath();
                    ctx.moveTo(lastp.x + 1.5, lastp.y + 1.5);
                ...
                if (mx < rx + md && mx > rx - md && my < ry + md && my > ry - md) {
                    var dx = Math.abs(mx - rx), dy = Math.abs(my - ry);
            ...
            if (plot) {
                ...
                var bx = plot.x > width - pw - 10 ? plot.x - pw - 7 : plot.x + 9, by = plot.y - 32;
            ...
            ig.xhr('backend/', {saveScreenshot: true, data: png}, callback);
...
ig.module('game.menus.title').requires('game.menus.base', 'game.menus.detailed-stats', 'game.ease').defines(function () {
    ...
    MenuTitle = Menu.extend({
        ...
        scroll: 0,
        background: new ig.Image('background/stars.jpg'),
        ztype: new ig.Image('title/ztype.png'),
        phoboslab: new ig.Image('title/logo1.png'),
        ship: new ig.Image('title/ship.png'),
        exhaust: new ig.Image('title/exhaust.png'),
        infoIcon: new ig.Image('ui/information.png'),
        glitchStripe: new ig.Image('title/glitch-stripe.png'),
        glitchCode: new ig.Image('title/glitch-code.png'),
        glitchJPEG: new ig.Image('title/glitch-jpeg.png'),
        glitchGraph: new ig.Image('title/glitch-graph.png'),
        glitchLog: new ig.Image('title/glitch-log.png'),
        ...
        draw: function () {
            ...
            this.background.draw(0, this.scroll - this.background.height);
            this.background.draw(0, this.scroll);
            this.phoboslab.draw((ig.system.width / this.scale - this.phoboslab.width) / 2+10, 50 - ig.ease.inOutQuad(this.transition, 0, 2800, 1));
...
ig.module('game.avenir-next').requires('impact.font').defines(function () {
    ...
    var METRIC_OFFSETS = {SX: 0, SY: 1, SW: 2, SH: 3, OFFSET_X: 4, OFFSET_Y: 5, ADVANCE: 6, ENTRIES_PER_GLYPH: 7};
    ig.FontAvenirNext = ig.Font.extend({
        letterSpacing: 1, height: 33, firstChar: 32, lastChar: 255, _widthForLine: function (s) {
            ...
            var sx = M[i + O.SX], sy = M[i + O.SY], sw = M[i + O.SW], sh = M[i + O.SH], offsetX = M[i + O.OFFSET_X], offsetY = M[i + O.OFFSET_Y], advance = M[i + O.ADVANCE];
...
ig.module('game.entities.explosion').requires('impact.entity', 'impact.entity-pool', 'game.entities.particle').defines(function () {
    window.EntityExplosionParticle = EntityParticle.extend({
        ...
        fadetime: 0.5,
        vel: {x: 60, y: 60},
        animSheet: new ig.AnimationSheet('sprites/explosion.png', 32, 32),
        init: function (x, y, settings) {
    ...
    window.EntityExplosionSpark = EntityParticle.extend({
        ...
        fadetime: 0.5,
        vel: {x: 60, y: 60},
        animSheet: new ig.AnimationSheet('sprites/spark.png', 48, 48),
        init: function (x, y, settings) {
    ...
    window.EntityExplosionHuge = ig.Entity.extend({
        ...
        fadetime: 1,
        alpha: 0,
        img: new ig.Image('sprites/explosion-huge.jpg', 512, 512),
        init: function (x, y, settings) {
            this.parent(x, y, settings);
            this.idleTimer = new ig.Timer();
        },
    ...
    window.EntityExplosionHugeGlitch = EntityExplosionHuge.extend({
        img: new ig.Image('sprites/explosion-huge-glitch.jpg', 512, 512),
        glitchJPEG: new ig.Image('title/glitch-jpeg.png'),
        glitchStripe: new ig.Image('title/glitch-stripe.png'),
        draw: function () {
            this.parent();
...
ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.avenir-next', 'game.entities.explosion').defines(function () {
    EntityEnemy = ig.Entity.extend({
        ...
        currentLetter: 0,
        targeted: false,
        font: new ig.FontAvenirNext('fonts/avenir-22-white-v2.png'),
        fontActive: new ig.FontAvenirNext('fonts/avenir-22-orange-v2.png'),
        speed: 10,
        ...
        numKillParticles: 20,
        numHitParticles: 5,
        soundHit: new ig.Sound('sounds/hit.ogg'),
        reticleTimer: null,
        reticle: new ig.Image('ui/reticle.png'),
        ...
        kill: function (silent) {
            ...
            if (!silent) {
                ...
                var px = this.pos.x - this.size.x / 2, py = this.pos.y - this.size.y / 2;
                for (var i = 0; i < this.wordLength.max; i++) {
                    ig.game.spawnEntity(EntityExplosionSpark, px, py, {vel: {x: 360, y: 360}});
                }
            }
        },
        cancel: function () {
...
ig.module('game.entities.enemy-missle').requires('game.entities.enemy').defines(function () {
    EntityEnemyMissle = EntityEnemy.extend({
        size: {x: 8, y: 15},
        offset: {x: 6, y: 7},
        animSheet: new ig.AnimationSheet('sprites/missle.png', 20, 26),
        explosionSound: new ig.Sound('sounds/explosion-small.ogg'),
        health: 4,
        speed: 35,
        targetTimer: null,
...
ig.module('game.entities.enemy-mine').requires('game.entities.enemy').defines(function () {
    EntityEnemyMine = EntityEnemy.extend({
        size: {x: 12, y: 12},
        offset: {x: 10, y: 10},
        animSheet: new ig.AnimationSheet('sprites/mine.png', 32, 32),
        explosionSound: new ig.Sound('sounds/explosion.ogg'),
        speed: 30,
        health: 6,
...
ig.module('game.entities.enemy-destroyer').requires('game.entities.enemy', 'game.entities.explosion').defines(function () {
    EntityEnemyDestroyer = EntityEnemy.extend({
        size: {x: 24, y: 34},
        offset: {x: 10, y: 8},
        animSheet: new ig.AnimationSheet('sprites/destroyer.png', 43, 58),
        explosionSound: new ig.Sound('sounds/explosion-large.ogg'),
        health: 8,
        speed: 20,
        shootTimer: null,
...
ig.module('game.entities.enemy-oppressor').requires('game.entities.enemy').defines(function () {
    EntityEnemyOppressor = EntityEnemy.extend({
        size: {x: 36, y: 58},
        offset: {x: 16, y: 10},
        animSheet: new ig.AnimationSheet('sprites/oppressor.png', 68, 88),
        explosionSound: new ig.Sound('sounds/explosion-large.ogg'),
        health: 10,
        speed: 15,
        shootTimer: null,
    ...
    EntityEnemyBullet = EntityEnemy.extend({
        size: {x: 2, y: 2},
        offset: {x: 8, y: 11},
        animSheet: new ig.AnimationSheet('sprites/bullet.png', 20, 24),
        health: 1,
        speed: 50,
...
ig.module('game.entities.emp').requires('impact.entity').defines(function () {
    ...
    window.EntityEMP = ig.Entity.extend({
        ...
        img: new ig.Image('sprites/emp.png'),
        sound: new ig.Sound('sounds/emp.ogg', false),
        init: function (x, y, settings) {
            this.parent(x, y, settings);
            this.idleTimer = new ig.Timer();
            this.sound.play();
...
ig.module('game.entities.player').requires('impact.entity', 'impact.entity-pool', 'game.entities.emp', 'game.entities.particle').defines(function () {
    EntityPlayer = ig.Entity.extend({
        animSheet: new ig.AnimationSheet('sprites/ship.png', 24, 24),
        targetAngle: 0,
        size: {x: 8, y: 8},
        offset: {x: 8, y: 8},
        angle: 0,
        targetAngle: 0,
        soundShoot: new ig.Sound('sounds/plasma.ogg'),
        soundMiss: new ig.Sound('sounds/click.ogg'),
        soundExplode: new ig.Sound('sounds/explosion-player.ogg'),
    ...
    EntityPlasma = ig.Entity.extend({
        ...
        animSheet: new ig.AnimationSheet('sprites/plasma.png', 96, 96),
        size: {x: 4, y: 4},
        offset: {x: 46, y: 46},
        distance: 100000,
        init: function (x, y, settings) {
...
ig.module('game.keyboard').requires('impact.font').defines(function () {
    ig.Keyboard = ig.Class.extend({
        background: new ig.Image('ui/keyboard.png'),
        empButton: new ig.Image('ui/emp-buttons.png'),
        pauseButton: new ig.Image('ui/pause.png'),
        font: new ig.Font('fonts/avenir-36-blue.png'),
        hoverImages: {
            leftEdge: new ig.Image('ui/key-edge-left.png'),
            rightEdge: new ig.Image('ui/key-edge-right.png'),
            normal: new ig.Image('ui/key.png')
        },
        multiBar: new ig.Image('ui/bar-blue.png'),
        multiIndicator: new ig.Image('ui/multi-indicator.png'),
        multiSounds: {2: new ig.Sound('sounds/multi-2.ogg'), 3: new ig.Sound('sounds/multi-3.ogg')},
        width: 0,
        height: 0,
        hoverKey: null,
        expectedKeys: null,
        keys: [['q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'], ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l'], ['z', 'x', 'c', 'v', 'b', 'n', 'm']],
        layout: {key: {width: 64, height: 106}},
        ...
        getCurrentKey: function (touches) {
            ...
            var touchX = (touch.clientX / documentScale) / this.drawScale, touchY = (touch.clientY / documentScale) / this.drawScale;
            ...
            if (this.expectedKeys) {
                ...
                for (var i = 0; i < this.expectedKeys.length; i++) {
                    ...
                    var keyX = kpos[0] * this.layout.key.width + this.layout.key.width / 2, keyY = kpos[1] * this.layout.key.height + this.layout.key.height / 2;
                    var dx = touchX - this.x - keyX, dy = touchY - this.y - keyY;
        ...
        drawFull: function () {
            ...
            if (this.hoverKey) {
                ...
                var x = (kpos[0] * this.layout.key.width - 19 + offsetX), y = this.y + kpos[1] * this.layout.key.height - 112;
                ctx.globalAlpha = 0.9;
                hoverImage.draw(x, y);
                ig.system.context.globalAlpha = 1;
...
ig.module('plugins.silent-loader').requires('impact.loader').defines(function () {
    ig.SilentLoader = ig.Loader.extend({
        endTimer: 0, fadeToGameTime: 300, end: function () {
            this.parent();
...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus.about', 'game.menus.game-over', 'game.menus.pause', 'game.menus.title', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'game.keyboard', 'game.xhr', 'game.ease', 'plugins.silent-loader', 'plugins.rise-loader', 'game.document-scanner', 'game.words.en').defines(function () {
    ...
    ZType = ig.Game.extend({
        font: new ig.Font('fonts/avenir-18-white.png'),
        fontTitle: new ig.Font('fonts/avenir-36-blue.png'),
        separatorBar: new ig.Image('ui/bar-blue.png'),
        idleTimer: null,
        spawnTimer: null,
        ...
        yScroll: 0,
        yScroll2: 0,
        gradient: new ig.Image('background/gradient.png'),
        stars: new ig.Image('background/stars.jpg'),
        grid: new ig.Image('background/grid.png'),
        music1: new ig.Sound('music/endure.ogg', false),
        music2: new ig.Sound('music/orientation.ogg', false),
        cancelSound: new ig.Sound('sounds/cancel.ogg'),
        spawnSound: new ig.Sound('sounds/spawn.ogg'),
        menu: null,
        mode: 0,
        score: 0,
        streak: 0,
        ...
        init: function () {
            ...
            if (window.Cocoon && window.Cocoon.Ad) {
                Cocoon.Ad.configure({android: {interstitial: 'ca-app-pub-8533552145182353/1042008307'}});
        ...
        spawnCurrentWave: function () {
            if (!this.wave.spawn.length) {
                ...
                var y = -30;
                this.spawnEntity(spawn.type, x, y, {healthBoost: this.wave.healthBoost, word: spawn.word}, true);
        ...
        setGame: function () {
            ...
            var sx = ig.system.width / 2 - 6, sy = ig.system.height - this.keyboard.height * this.keyboard.drawScale - 30;
    ...
    ZType.WAVES = {
        MOBILE: {
            ...
            healthBoost: 0,
            speedIncrease: 1.01,
            types: [{type: EntityEnemyOppressor, count: 0, incEvery: 9}, {
        ...
        DESKTOP: {
            ...
            healthBoost: 0,
            speedIncrease: 1.05,
            types: [{type: EntityEnemyOppressor, count: 0, incEvery: 7}, {
                type: EntityEnemyDestroyer,
                count: 0,
                incEvery: 3
            }, {type: EntityEnemyMine, count: 3, incEvery: 1}]
